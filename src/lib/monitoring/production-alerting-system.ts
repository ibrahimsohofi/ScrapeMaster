...
export interface SSLConfig {
  ...
  chainPath?: string;
  autoRenewal: boolean;
  provider: 'letsencrypt' | 'custom' | 'cloudflare' | 'zerossl';
...
export class EnterpriseMonitoringSystem extends EventEmitter {
  ...
  private async sendToExternalServices(alert: EnterpriseAlert): Promise<void> {
    ...
    if (this.externalConfig.grafana.enabled) {
      promises.push(this.sendToGrafana(alert));
    }
  ...
  private async sendToDatadog(alert: EnterpriseAlert): Promise<void> {
    this.logger.info('Sent alert to Datadog', { alert: alert.id });
  }

  private async sendToGrafana(alert: EnterpriseAlert): Promise<void> {
    this.logger.info('Sent alert to Grafana', { alert: alert.id });
  }

  private async sendToPrometheus(alert: EnterpriseAlert): Promise<void> {
    this.logger.info('Sent alert to Prometheus', { alert: alert.id });
  }
